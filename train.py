...
def fit_one_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    # tensorboardX
    writer.add_scalars('scalars/loss', {'train': total_loss / (epoch_size + 1), 'val': val_loss / (epoch_size_val + 1)},
                       epoch)
...
if __name__ == "__main__":
    ...
    if hyp.get('lr_scheduler') == 'plateau':
        lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer,min_lr=0.0001,factor=0.5,patience=2,verbose=True)
